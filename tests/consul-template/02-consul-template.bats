...
@test "templates using 'range services' function are well rendered" {
  ...
  status=$(consul-template -consul-addr ${consul_service}:8500 -config /etc/consul-template/conf -once;echo $?)
  [ "$status" -eq 0 ]
...
@test "templates using 'keyOrDefault' function  are well rendered" {
  # Declare the service
  key="tests/keyOrDefault"
  value="tests/keyOrDefault/value"
  ...
  # Check we can find the address and port we registered
  status=$(grep "keyOrDefault: $value" /tests/consul-template/tests.conf  2>&1 > /dev/null;echo $?)
  [ "$status" -eq 0 ]
}