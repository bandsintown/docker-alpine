...
# https://github.com/CircleCI-Public/cimg-shared/blob/main/gen-dockerfiles.sh
...
parse_template_variables () {
    ...
    # Dependencies
	sed -i.bak 's!%%S6_OVERLAY_VERSION%%!'"${s6_overlay_version}"'!g' "./versions/${vgVersionFull}/${variantPath}Dockerfile"
	sed -i.bak 's!%%GODNSMASQ_VERSION%%!'"${godnsmasq_version}"'!g' "./versions/${vgVersionFull}/${variantPath}Dockerfile"
	sed -i.bak 's!%%CONSUL_TEMPLATE_VERSION%%!'"${consul_template_version}"'!g' "./versions/${vgVersionFull}/${variantPath}Dockerfile"
	sed -i.bak 's!%%CONSUL_VERSION%%!'"${consul_version}"'!g' "./versions/${vgVersionFull}/${variantPath}Dockerfile"
}
